...
remove_project_specific_files() {
    ...
    rm -r ./config/dev.yml 2>/dev/null
}
...
copy_content() {
    ...
    cp -r "${_dir}"/../config "${_dir}"/"${_PROJECT_NAME}"
    cp -r "${_dir}"/../scripts "${_dir}"/"${_PROJECT_NAME}"
    cp "${_dir}"/../package.json "${_dir}"/"${_PROJECT_NAME}"
    cp "${_dir}"/../package-lock.json "${_dir}"/"${_PROJECT_NAME}"
    cp "${_dir}"/../.babelrc "${_dir}"/"${_PROJECT_NAME}"
}
